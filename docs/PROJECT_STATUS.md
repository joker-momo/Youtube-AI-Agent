...
Last updated: 2026-07-06d (SEO/CTA quality pass + render-QA fix + Codex-work recovery, branch `feat/longform-script-pacing-chapters`, 14 commits, all verified. **Long-form script/SEO (d7f86be):** CONTENT ANTI-REPETITION & PACING block in `_chatgpt_script_prompt` (no idea twice, no mid-video recap filler, expand with depth not repetition, forward momentum ~30-45s, one beat per section, varied mechanism openers — sample gens verified 0 recap filler and varied "La razón es…/Detrás de esto hay…" openers), and the SEO description's Chapters section now enforces YouTube's auto-chapter contract (first at 00:00, ≥3, ascending ≥10s apart, short specific labels) — sample SEO gen passed all five checks. Depth kept: NO length cap, no 8-12min cut (45+/60+ niche wins on depth per competitor teardown). **Shorts title+CTA (99e30af..23ac210):** titles lead with the A/B-winning question/contrarian device; the spoken funnel CTA now names the parent long video's content in the model's own words ("Descubre el error del aceite en ayunas en el canal." — verified live), driven by source_video_title threaded into the script prompt; the deterministic gate only requires the channel direction ('canal') so prompt and validator can never disagree again (bug-484 chain: three prompt-side fixes were being silently rejected by a hardcoded validator expectation, then force-repaired back to the generic phrase — root-caused via llm_history.jsonl evidence); funnel.cta_max_words 8→12 (config-driven via funnel_cta_max_words()) because the 8-word cap was what forced telegraphic fragments; CTA-only isolated stage tests (no render) verified 2/2 first-shot clean. **Render QA (794831d, bug-485):** PLACEHOLDER_USED was blocking Shorts whose placeholder background is fully covered by real tracks of the ENFORCED compiled schedule (the placeholder never renders — caused 4/7 render failures on rotating scenes); such scenes now downgrade to PLACEHOLDER_BACKGROUND_COVERED_BY_SPAN warning, replay of the real failed artifacts passes; long-form/uncovered/report_only keep the hard gate. **Incident + recovery (592d3f1):** a concurrent "trinity" agent stashed + reset the shared tree at 16:04, wiping ALL uncommitted work including Codex's entire bug-474..479 fix set (renders regressed to 3 missing spans immediately); recovered verbatim from stash@{0} (stash kept as backup), union-merged .wolf logs, 368 tests pass, committed so resets can't destroy it again. Long-form thumbnail flow stays at the reverted ChatGPT-gen original per owner instruction (v1.4/v1.5 anti-AI prompt + Option-A overlay were fully rolled back earlier today). Final proof: idea-01 short rendered end-to-end in 13 min with title-B + natural CTA, delivered. Known nit: SEO title may end with "…" when the model trims to the 60-char cap. Worker prompt-code changes require a worker restart with the run.sh env (never bare).)

Last updated: 2026-07-06b (Shorts anti-slideshow polish + thumbnail removal. **Slideshow feel (user report):** despite per-scene Ken-Burns, shorts read as a slide deck because ShortVideo hard-cut scenes (plain back-to-back `<Sequence>`, no transition — long-form has SCENE_XFADE, shorts had none) and baked ChatGPT graphic cards sat near-frozen. **Fix A — scene cross-dissolve (no timeline shrink):** `ShortVideo.tsx` now wraps each scene in `SceneCrossfade` (opacity ramp over `SHORT_SCENE_XFADE`=7f) and extends only the RENDER length past the boundary while `cursor += durFrames` keeps every scene start on the true grid — so the single narration track + subtitles never desync and total duration is unchanged (deliberately NOT Remotion `TransitionSeries`, whose overlaps shrink the timeline → audio desync + duration-mismatch). **Fix B — graphic push-in:** a baked card can't animate its pixels, so `ShortBackground` takes an `isGraphic` flag and forces a slow push-in scale 1.0→1.10 (overriding the weak `text_pop` drift); the card margins absorb the crop. Verified on a fresh real render: a mid-transition frame shows ghosted (semi-opacity) keyword text = two scenes blending; the s04 card is visibly larger at scene end than start; audio stayed synced. tsc clean; +4 source-assertion tests (`tests/test_shorts_transition_polish.py`, kept separate from the mid-migration `test_shorts_remotion_contract.py`). **Thumbnail removal from shorts pipeline:** deleted `short_thumbnail_builder.py`, 3 dead `thumbnail_fn` in `orchestrator/worker.py`, the renderer thumbnail-copy + seo `thumbnail_path`/`thumbnail_text` (now ""), `SHORT_THUMBNAIL_FILE`, `thumbnail_composition` (render_props/pipeline), `thumbnail_meta`/`thumbnail_emotion` (media/performance_memory) — Shorts have no cover deliverable; kept `auto_thumbnail_image_stage` (long-form), `thumbnail_source` (shared), `_finish_render_artifact` short-skip (bug-479). 248+153 tests pass, 0 new ruff. **buglog bug-474..479 all marked FIXED (last_seen 2026-07-06).** Note: Remotion source changes need no worker restart (render subprocess bundles `remotion/src` live) but must bypass the fast-path (delete `outputs/video.mp4`) or the old file is reused. Remaining slideshow levers (not done): more real 45+ b-roll vs stills, per-word kinetic subtitles, and the enforced VisualTimeline path would need its own crossfade.)

Last updated: 2026-07-06 (Shorts render-completion CHAIN — one "short won't render" job was 6 stacked blockers + 1 provider bug; fixing all rendered it end-to-end (video.mp4 h264 1080×1920 aac 21.7s, delivered to Telegram). Also a perf optimization. Surfaced in order, each masking the next: **bug-474** ChatGPT web refuses a big strict-schema scenes JSON claiming per-message size limits in TWO shapes (refusal prose OR valid `{"error":…,"scenes":[]}`) → old text-only `is_provider_error_text` missed it → flowed into creative QA as `empty_scenes`, and the repair loop GREW the prompt (28.8k→30.4k) reinforcing the refusal until retry-collapse. Fix: payload-aware `is_size_refusal_response` + ONE compact/minified retry addendum + `ChatGPTSizeRefusalError` (off the creative budget); scene prompt now demands minified one-line JSON + forbids size-refusal/error-object/empty/split. **bug-475** `visual_beats` `missing_selected_plan:vs04` — span-level local QA rejected all `vs04` finalists but `background` later made a strong-match `s04.mp4`; planner had no path to adopt it. Fix: `revalidate_failed_spans_with_fallback` re-runs the SAME local+semantic gate on the background fallback (reuses `_candidate_verdict`/`_semantic_records`/`_qa_verdict`), rescues ONLY on a genuine PASS (never CAPABILITY_REDUCED). Real data: s04 fallback is a young-person clip (`age_band_45_plus` CONTRADICTED) → correctly DECLINED (no off-audience footage) → video could only complete via generated-image. **bug-477** (the real completion blocker) `regen_fallback_backgrounds` kept the semantically-rejected stock instead of forcing AI — scene_resolver Tier-1 strict-stock re-selects the SAME rejected clip (no view of the QA rejection). Fix: force `asset_strategy=ai_image_preferred` for rejected native scenes so s04/s06 generate on-brand ChatGPT images (→ `_resolved_generated_image` → planner adopts `generated_image_fallback`). **bug-476** `LocalVisualAnalyzer` decode off-by-one — container duration overhangs the last real frame so the final sample seek failed and spuriously decode-failed a VALID clip; fix: 2-frame tail-guard + clean-EOF (would otherwise block valid 45+ fallbacks too). **bug-478** shorts inherited long-form intro/outro (`branding.enable_intro_outro` on since 2026-07-01) → `render.duration_sec` 39.9 vs the scenes-only 21.9 MP4; fix: shorts render_props force intro/outro=0 (Short composition renders scenes only). **bug-479** shared `_finish_render_artifact` requires a ChatGPT thumbnail (legacy Remotion fallback removed 2026-06-30) but Shorts have NO cover deliverable; fix: skip the thumbnail gate for shorts (long-form still hard-fails). **Optimization** (Codex bridge): process-level `_HF_MODEL_CACHE` + `local_files_only`-first loader for SigLIP/Grounding DINO in `visual_local_qa` — loads each model once per worker + skips HF metadata round-trips, quality gates unchanged. **Verify:** all touched suites green (visual_local_qa/analysis/semantic/sequence_qa + shorts_build + shorts_audio + render_handoff + render_segments/command), ruff clean on all touched files; live worker-restart rerun rendered the short end-to-end (visual_beats PASS, s04=ChatGPT infographic + s06=lifestyle image, no rejected footage). Replied all 3 Codex bridge tasks (fixed-pending-codex): size-refusal, semantic-model-startup optimization, visual-beats-fallback. buglog bug-474..479 + cerebrum "chain" learning. Right shorts render entrypoint = `POST /shorts-studio/jobs/<job>/ideas/render` (video-only); a valid video.mp4 re-renders via fast-path once the blocking gate is fixed.)

Last updated: 2026-06-29d (Professional-polish + UI-path render fixes — a COMPLETE 12m16s video delivered end-to-end through the worker/UI pipeline, MeloTTS voice, auto-sent to Telegram. **Visuals:** (1) **hook → ChatGPT gen card**: added `hook` to `visual.spans.GRAPHIC_LAYOUTS` so the hook is a designed brand card (also dodges the weak-stock-match block); (2) **graphic cards carry FULL content + brand**: `graphic_images._graphic_prompt` now bakes title+bullets+body+cta from `layout_payload` (was only the short `on_screen_text`), pins the channel font (reads `style-dna.json` typography=Manrope) and injects the brand palette+mood (cream #F6F1E8 / green #2F6B57 / terracotta #D98C5F / yellow #F5C24B / dark-green text #26332F) at high contrast — killed the generic navy/white/yellow clickbait look; `ChannelVideo.tsx` skips the Remotion text overlay on graphic scenes (image already has the text); (3) **Pexels VIDEO gate fix**: Pexels video objects carry NO tags/alt so the strict on-topic gate rejected EVERY video → all scenes fell to photo; query-ranked video now counts → **37 real videos** (was 0); critical-scene weak-match downgraded error→warning (a query-ranked video is on-topic + beats blocking/placeholder); (4) **persistent logo**: moved per-scene → one top-level overlay in `ChannelVideo.tsx` so it stops blinking on scene cuts; (5) **scene crossfade**: replaced the per-scene dip-to-black with a sync-safe cross-dissolve (scenes overlap `SCENE_XFADE=15f`; all time-based animation uses `localFrame` so subtitles/Elena/audio stay on the continuous audio timeline); (6) **Elena full-span + loop crossfade**: the cue spans the whole scene (renderer loops the ~8.8s clip) and two phase-offset copies cross-dissolve at each seam (`ChannelElenaPresenter.tsx`) so the talking head never hard-cuts. **Pipeline/UI fixes:** `orchestrator/worker.get_channel_path` default → repo-relative (was Docker `/app/...`) so the worker reads the real `visual.elena.enabled`; `render_review.run_render_stage` resets `render_progress.json` at stage start + dashboard shows "Preparing render…" until real frames (the bar was frozen at a stale 100% during the long prepare_assets); `notify_job_done_with_files` reads `ARTIFACT_VIDEO` (`outputs/video.mp4`) not a hardcoded `video.mp4` → the finished video actually sends to Telegram (auto-compresses 1GB→<50MB for the bot cap); dashboard idle-poll fix — the heavy /timeline + render-progress pollers gate on the job actually running, so a finished job stops the constant 2s polling (no "reload" feel, lighter machine). **Verify:** edited modules ruff+mypy clean; full suite **1579 pass / 7 PRE-EXISTING fail** (shorts-pipeline ×2 + dashboard-7-tabs + idea/job/tts-spec — all fail on HEAD too, in earlier-work areas, NOT this polish). Bug log + cerebrum updated (`.wolf` is gitignored). Memory: `feedback_close_unused_bg_tasks`, `project_elena_loop_crossfade` (now implemented).)

Last updated: 2026-06-29c (Quality-first script length — min 11 min, NO max, scenes PRESERVE content. User: "ko gò bó cứng nhắc về thời gian, nhấn mạnh chất lượng nội dung … min 11 phút, ko có ngưỡng max". Root cause found via the new Transcript Lab tool: for a published job (`OxkfPqKxYp4`) the `script.json` narration was **3832 words** but the rendered video only **1699 words** (44%) — the active **sharded** scenes stage (`SCENES_SHARDED_GENERATION=1`) condensed it to a fixed ~48 scenes / 840s, dropping 283/359 narration sentences of substantive content. The `script_word_budget_multiplier=1.43` rested on a **stale** "LLM under-produces" assumption (current models over-produce). **Fix (bug-402):** `_chatgpt_script_prompt` now uses a MINIMUM-only word floor from `content_format.duration_sec_min` (660s=11min → 1320w @120wpm) with **NO max** + "QUALITY OVER BREVITY: develop every idea fully"; removed the 1.43 inflation. Sharded `_chatgpt_scenes_plan_prompt` `target_scene_count` is now **content-driven** (~45 words/scene, floor `scenes_count_min`, no cap) and `_chatgpt_scenes_batch_prompt` must "reproduce faithfully, do NOT summarize, split into MORE scenes"; monolithic fallback mirrored (PRESERVE CONTENT / "ADD scenes" not "reduce length"). `channel.yaml`: `duration_sec_min` 720→660, `duration_sec_max` 900→1800 (soft, no hard trim); `video_job_creator` idea bounds 360/1200→660/1800. **Verified on real config:** 1320w→40 scenes/11min (floor); **3000w→67 scenes/25min** (was capped ~48/14min) — no more compression. Tests: `tests/test_script_length_quality_first.py` (6 new) + 121 regression pass. Next: review competitor scripts via Transcript Lab, then tune further. Tool: `tools/transcript_lab/` (standalone, port 8750, fetch YT transcripts; NOT in main flow).)

Last updated: 2026-06-29b (Elena SIMPLE per-scene rule — frequency REMOVED — + full 12m16s long video delivered with the MeloTTS Elena voice. User: "chia làm 3 dạng … bỏ hẳn tính tần xuất". Rewrote `build_elena_cues` (`src/video_agent/visual/elena.py`) from the band-aware cadence planner to **ONE cue per ELIGIBLE scene** — no cadence / gap / visibility-band / asset-alternation. **3 categories by layout** (+ annotation override): **HIDDEN** (no cue) = `{checklist,quote,cta}` (uncoverable) + `elena.mode=hidden` + wordy `warning`; **LARGE** (talk-emphasis, 384px) = `hook` + eligible `warning` (≤8-word label or annotation); **CIRCLE** (talk-neutral, 240px) = `subtitle` / other. Cue starts at scene start, spans the scene capped to clip playable (10s−1s trim = 9s); 1s `source_trim_frames`; `qa.verdict` always PASS (band gate gone). Removed `_TARGET_GAP_SEC/_MIN_GAP_SEC/_MAX_GAP_SEC/_TARGET_APPEARANCE_SEC/_VISIBLE_BAND_PCT/_APPEARANCE_BOUNDS_SEC` + `_next_variant`/`_appearance_frames` + the while-loop placement + band-QA errors. Tests rewritten (`test_elena_band_warning.py` + `test_long_elena.py`) → 32 pass; ruff+mypy clean. **Delivered:** `jobs/si-tienes-mas-de-60-anos-…-032542/video.mp4` — **1.07 GB, 1920×1080 h264+aac, 12m16s (735.7s)**, MeloTTS Elena voice, **42 Elena cues / 51.4% visible**; stills @4.5s/35s/52.9s confirm large@hook, hidden@quote, circle@subtitle. **Two render bugs fixed en route:** **bug-393** graphic `image_ref` 404 render-hang at frame 349 — `graphic_images` now publishes PNGs into `remotion/public/jobs/<id>/assets/` AND carries the `jobs/<id>/` prefix (idempotent/resumable); **bug-394 (UNRESOLVED)** — the worker/UI render_props came out `elena_cues=None` despite every injection precondition passing in isolation, so Elena was injected via the proven **direct `render_with_remotion`** path. The pure-UI Elena path stays blocked pending a root-cause fix of the worker render_props build. Note: the queue worker imports modules at startup → a code change to elena.py/pipeline.py needs a worker restart to take effect.)
