...
def _chatgpt_seo_prompt(
    ...
    return "\n".join(
        [
            ...
            "    - REQUIRE three distinct title devices across the 3 variants, one each: (1) curiosity/question — a hidden mistake/step tied to THIS video's actual topic, e.g. patterns like 'el [X] que casi nadie [verbo]', 'lo que pasa cuando/después de [Y]', 'por qué [Z] no funciona como crees'; (2) contrast/contrarian — a specific myth from THIS topic framed as '(no es [culprit del video])' or 'no es lo que piensas sobre [tema]'; (3) keyword-first/direct-payoff — the searchable topic keyword stated plainly with a concrete payoff (SEO safety net). A concrete number ('N errores', 'N señales', 'N hábitos') or ONE power word (silencioso, sorprendente, sencillo, rápido, oculto, inesperado, decisivo) may sharpen any device but does not replace it.",
            ...
            "    - EVERY title must carry a personal stake (a concrete consequence/decision for THIS viewer, not a description-only label of the topic) plus a concrete payoff (what they gain by watching). Reject description-only titles — a title that only names the topic without stake or payoff is incomplete.",
            "    - ⚠️ HONEST CTR ONLY: NO fake authority/credentials ('cardiólogo advierte', 'enfermera revela'), NO fear or death claims ('detiene tu corazón', 'te está matando'), NO 'milagro/cura/garantiza'. Mention scientific/medical authority or a specific statistic only when the approved script material actually supports it — otherwise keep the claim to what is supported by the video's own content. The curiosity MUST be payable by the actual video content.",
            "  • thumbnail_text: target 3-5 words (hard allowance 3-6 words) ALL-CAPS Spanish, complementary to the title. It is a STANDALONE MICRO-PROMISE: a viewer aged 45-75 must understand, without reading the YouTube title, what familiar problem/object the video concerns and what practical value they get.",
            ...
            "• For thumbnail_text, use 3-6 words, all caps, Spain-natural Spanish, strong but not exaggerated." if is_spain else "• For thumbnail_text, use 3-6 words, all caps, natural Spanish for the configured locale, strong but not exaggerated.",
