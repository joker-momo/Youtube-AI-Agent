...
def _chatgpt_scenes_prompt(
    ...
    prompt_parts = [
        ...
        "- ⚠️ VISUAL RHYTHM (critical for retention): the graphic layouts (hook/checklist/warning/quote/cta) render as full design cards. Two or more cards back-to-back feel like a static slideshow and lose viewers. NEVER place two graphic-layout scenes consecutively — separate EVERY graphic scene with at least one (ideally two) layout=\"subtitle\" narrative scene(s) that play over moving video. This applies right after the scene-01 hook too: scene-02 onward must be \"subtitle\" until the next genuine card moment. Most scenes should be \"subtitle\"; spread checklist/warning/quote sparingly across the whole script.",
