...
def _chatgpt_seo_prompt(
    ...
    return "\n".join(
        [
            ...
            "  • thumbnail_text: 3-7 words ALL-CAPS Spanish hook. Pick the strongest device for THIS topic (competitor-proven, in priority order):",
            "    1. IMPERATIVE + AGE: a command plus the age frame — 'HAZ ESTO DESPUÉS DE LOS 50', 'EVITA ESTO A LOS 60'.",
            "    2. NEGATION COMMAND: contradict the obvious habit — 'NO NECESITAS CAMINAR', 'NO ES EL PAN'.",
            "    3. COUNT: a concrete number from the video — '6 COSAS ANTES DE DORMIR', 'SOLO 1 CUCHARADA'.",
            "    4. EMOTIONAL HOOK: the classic device — 'ADIÓS AL INSOMNIO', 'TU PLATO TE HABLA'.",
            "    - Across the 3 variants use DIFFERENT devices (never three emotional hooks, never two negations).",
            "    - MANDATORY: at least ONE of the 3 variants uses device 1 (IMPERATIVE + AGE) — every top competitor puts the age on the thumbnail.",
            "    - Still honest: no fake authority, no fear/death, no miracle claims.",
