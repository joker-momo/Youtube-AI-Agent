...
def _chatgpt_script_prompt(channel_config: dict[str, Any], idea: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "• MECHANISM: for each main point, briefly explain WHY it happens in plain physiology ('esto ocurre porque…'). The best videos teach the cause, not just the action. Keep it simple, no jargon.",
...
def _gemini_scenes_qa_batch_prompt(
    ...
    return "\n".join(
        [
            ...
            "",
            "LAYOUT & VISUAL RHYTHM QA (add to issues; NEEDS_REWORK if serious):",
            "- Renderable layout: every non-subtitle layout MUST be one of hook, checklist, warning, quote, cta, stat, steps, comparison, myth, plate_map, recipe_snapshot, quote_portrait, evidence_nugget, do_dont. Flag any other value (it would render as a plain subtitle, not a card).",
            "- Rhythm: NO two graphic-layout scenes back-to-back (separate each with ≥1 subtitle scene); also flag very long runs (6+) of consecutive subtitle scenes with no card moment.",
            "- Grounding: each card's layout_payload text (title/body/bullets) must appear in that scene's narration/caption — flag invented or paraphrased card text (Python will otherwise downgrade it to subtitle).",
            "- Semantic fit: warning = a mistake/risk to AVOID (not a positive to-do list); quote/quote_portrait = one short memorable sentence (not bullets); stat/evidence_nugget = a real number/fact; comparison = two neutral options while do_dont = one worse vs one better. Flag mismatches (e.g. a warning rendered as a green-check checklist).",
            "",
            "Channel config:",
            _json_block(channel_config),
            "",
