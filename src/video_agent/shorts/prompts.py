...
_SPANISH_NATURALNESS = (
    "SPANISH NATURALNESS (es-ES, must sound native — NOT machine-translated):\n"
    "- Write each line the way a Spaniard would actually say it aloud, not a literal word-by-word rendering.\n"
    "- Use complete idiomatic phrasing with the natural subject/object; avoid clipped calques.\n"
    "- The words must stand alone: never rely on the on-screen image to make a line understandable.\n"
    "- Avoid literal expressions a native could misread (e.g. 'luchar en cama' reads as physically fighting in bed).\n"
    "- Bad -> Good (fix this kind of machine-translation feel):\n"
    "    'Lee algo suave' -> 'Lee algo tranquilo'\n"
    "    'Respira o escucha' -> 'Respira lento o escucha algo tranquilo'\n"
    "    'Vuelve con sueño' -> 'Vuelve a la cama cuando tengas sueño'\n"
    "    'Deja de mirarla' -> 'Deja de mirar la hora'\n"
    "    'deja de luchar en cama' -> 'deja de forzar el sueño' / 'no mires el reloj'\n"
    "- Lines should flow as one calm thought with everyday connectors, not disconnected slide captions.\n"
)
...
def short_script_prompt(
    ...
    *,
    ...
    return (
        ...
        "AUDIO COUNT INTEGRITY (critical, non-negotiable):\n"
        "- If the SPOKEN narration announces a number of items (e.g. \"Cinco ajustes\", \"3 errores\"), the AUDIO must then verbalize every one of those items. Never announce \"Cinco\" and only speak \"Uno, Dos, Tres\" — that breaks the promise and leaves an abrupt audio gap (a hard QA blocker).\n"
        "- Moving detail to on-screen text means trimming WORDS within an item, never dropping whole announced items out of the audio.\n"
        "- Preserving the idea's number applies to the TITLE and on-screen text; the SPOKEN line must match what the audio actually delivers.\n"
        "- If all announced items cannot fit the spoken-word budget for this duration, do NOT state the number in the spoken line. Use a non-numeric framing instead (e.g. \"Algunos ajustes\", \"Estos ajustes\", \"Cambios sencillos\") — never promise a count the audio will not complete.\n"
        "- The spoken announced count and the number of items verbalized in the audio MUST match exactly.\n\n"
        "OUTPUT RULES:\n"
...
def short_seo_prompt(
    ...
    *,
    ...
    title_formulas = (
        ...
        "- STANDALONE MICRO-PROMISE: the title must NAME the concrete topic object "
        "(CAFÉ, SAL, PAN, ESPALDA…) plus the pain, action, or payoff. NEVER hide the "
        "content behind a bare \"esto\"/\"eso\": a 45+ viewer who cannot tell WHAT the "
        "Short is about will not stop scrolling, so context-free fragments are banned.\n"
        f"- Use ONE of these 4 proven Spain-Spanish formulas — whichever fits the script. "
        f"The age frame ({min_age}) is OPTIONAL: drop it when the topic words need the space:\n"
        f"  1) WARNING (miedo a hacer algo mal): \"¡Error al [acción]! (Mayores de {min_age})\" "
        f"— p.ej. \"¡Error al comer fruta! (A los {min_age}+)\", \"¿Espalda rígida? El error al agacharte\"\n"
        ...
        f"  4) CALL OUT (llamar al público + tema concreto): \"Si tienes más de {min_age}, [acción + tema]\" "
        f"— p.ej. \"Si tienes más de {min_age}, revisa tu sal\", \"Si tienes más de {min_age}, cuida tu café\" "
        "(the call out MUST end with the concrete topic).\n"
    ...
    return (
        ...
        "- End with ONE short, low-friction question (no yes/no). Prefer a concrete either/or the viewer can answer in a word or two (e.g. \"¿Qué te cuesta más: dormirte o volver a dormir?\") over a vague open question that demands a long reply. Keep the whole comment to 1-2 short sentences and never ask two questions.\n"
...
def short_scene_prompt_v6(
    ...
    *,
    ...
    return (
        ...
        "- asset_strategy: 'stock_ok' (default), 'ai_image_preferred' for hard-to-find specific visuals, 'graphic_fallback' only when the scene must become a ChatGPT-generated graphic/image asset.\n"
        ...
        "Preserve ALL of the SCRIPT's content across the scenes, beat by beat: every claim, "
        "every qualifier that carries the point (e.g. 'realmente', 'sin cambiar nada'), the "
        "payoff sentence, each numbered step, BOTH hook sentences, and the CTA's specific "
        "topic phrase MUST be spoken. Prefer the script's exact wording; you MAY lightly "
        "reword for natural spoken flow, but NEVER drop, summarize away, or change the meaning "
        "of any beat. The scene_qa source-fidelity gate hard-blocks DROPPED or CHANGED content "
        "(a faithful paraphrase that keeps every beat passes), so compressing content out only "
        "makes the Short fail.\n"
        ...
        "If a script sentence is too long/dense for one scene's pacing, SPLIT it across "
        "consecutive scenes at a natural clause boundary (after a comma or connector), "
        "keeping every clause and point — nothing dropped. Add MORE scenes; never compress "
        "into fewer, denser ones.\n"
        "⚠️ NEVER drop, summarize away, or reduce a beat to a bare fragment to fit a word cap "
        "or a timing target. BAD (fails QA): script 'Primero, mide cuánto aceite usas realmente, "
        "sin cambiar nada' → 'Mide lo que usas.' (drops 'realmente' and 'sin cambiar nada', the "
        "whole point). CORRECT: keep it as one scene, or split at the clause boundary into 'Primero, "
        "mide cuánto aceite usas realmente,' + 'sin cambiar nada.' — no content lost.\n"
        "Captions, on_screen_text, and layout_payload are ADDITIONAL context; NEVER move "
        "spoken script words out of narration into them to shorten the spoken line.\n"
        "Speak full natural sentences, not telegraphic 3-4 word fragments — fragments read as "
        "low quality and fail the product-quality gate.\n"
        "Scene narration must sound natural in es-ES, a real spoken sentence, not keyword fragments.\n\n"
        "OUTPUT RULES:\n"
        "Return exactly ONE raw valid JSON object.\n"
        "No markdown fences.\n"
        "No commentary.\n"
        ...
        "SCENE NARRATION LENGTH & PER-SCENE WORD CAPS (these are SPLIT triggers, NOT compression):\n"
        "- Keep every scene's narration faithful to the script — preserve all content. Do NOT compress, drop, or summarize away content to hit the caps.\n"
        "- The caps below signal when to SPLIT: if a script sentence exceeds a scene's cap, break it at a clause boundary into consecutive scenes, keeping every clause.\n"
        "- Captions, on_screen_text, and layout_payload are ADDITIONAL; never move spoken script words into them to shorten a line.\n"
        "- Do not invent new claims, and NEVER remove any clause, qualifier, or the payoff.\n"
        "- A script beat that exceeds a cap is SPLIT across scenes, never trimmed to fit the cap.\n"
        ...
        "  - graphic_stat: max 5–7 spoken words\n"
        "  - graphic_myth: max 7–9 spoken words\n"
        "  - graphic_do_dont: max 7–9 spoken words\n"
        "  - graphic_recipe_snapshot: max 7–9 spoken words\n"
        ...
        "- graphic_checklist    -> 2-4 short action items under one instruction.\n"
        "- graphic_step_list    -> 2-4 numbered steps; a step may carry an optional short \"time\" (e.g. \"10 min\") for time-block routines.\n"
        "- graphic_label_callout -> label-reading moments (fibra, azúcares, sal, proteína, por 100 g).\n"
        "- graphic_comparison   -> two NEUTRAL choices such as MEJOR vs CUIDADO without fear language.\n"
        "- graphic_stat         -> ONE memorable number/quantity the narration centers on (\"3 pasos\", \"2 veces al día\", \"80%\"), including credible evidence facts tied to age/health (\"después de los 60\", \"masa muscular\").\n"
        "- graphic_myth         -> the narration corrects a misconception: mistaken belief (mito) + correction (realidad).\n"
        "- graphic_do_dont      -> a WORSE choice vs a clearly BETTER one (\"esto no, mejor esto\"); use graphic_comparison when the two options are neutral.\n"
        "- graphic_recipe_snapshot -> a concrete PRACTICAL meal example (e.g. yogur + avena + fruta) as 2-3 real foods.\n"
        "- graphic_warning      -> 2-4 mistakes/risks to AVOID (calm caution markers, never fear language).\n"
        "These 10 are the ONLY selectable graphic layouts. LEGACY LAYOUT MIGRATIONS:\n"
        "an age/health evidence fact (formerly graphic_evidence_nugget) is now a graphic_stat;\n"
        "a time-block routine (formerly graphic_routine_split) is now a graphic_step_list whose\n"
        "steps carry the optional \"time\" field; graphic_quote_portrait is reserved for internal\n"
        "repair of one-item lists — never select it yourself.\n"
        "Use a graphic for the highest-value knowledge moment. Checklist/explainer\n"
        "Shorts with a real list, portion rule, plate/component breakdown, label fact,\n"
        "or comparison MUST include at least one graphic_* scene. Most other scenes\n"
        "stay stock short_* layouts. Graphic scenes are explanatory bursts, not slides:\n"
        ...
        "- graphic_stat: target 2.5–3.5 sec, hard max 4.0 sec.\n"
        "- graphic_myth: target 3.0–4.0 sec, hard max 4.5 sec.\n"
        "- graphic_do_dont: target 3.5–4.5 sec, hard max 5.0 sec.\n"
        "- graphic_recipe_snapshot: target 3.0–4.5 sec, hard max 5.0 sec.\n"
        "- graphic_warning: target 3.0–4.0 sec, hard max 4.5 sec.\n"
        ...
        "- Pick the 1–2 HIGHEST-VALUE knowledge moments for graphics: prefer graphic_label_callout for \"primer ingrediente\" and graphic_comparison for \"fibra / azúcar / jarabes\". Use graphic_checklist for action lists. Use graphic_plate_ratio for plate/portion/component breakdowns such as palm-size portion, 1/2 vegetables, 1/4 protein, or bread/rice/potato share. Render setup beats as realistic short_tip or short_myth scenes, but do NOT omit all graphics from a checklist/explainer.\n"
        ...
        "\"Luz baja\", \"Lista de pendientes\"], \"footer\": \"...\"} (2-4 items, HARD limit — "
        "a fifth item means the scene must be split or simplified, never squeezed in).\n"
        "graphic_step_list: {\"title\": \"EN 3 PASOS\", \"steps\": [{\"label\": \"1\", \"text\": "
        "\"Mira la etiqueta\"}, {\"label\": \"2\", \"time\": \"5 min\", \"text\": \"Busca fibra\"}], "
        "\"footer\": \"...\"} (2-4 steps; \"time\" is optional per step, short like \"10 min\").\n"
        ...
        "\"footer\": \"...\"} (title = the ONE number/phrase, body = short label; no bullets).\n"
        "graphic_myth: {\"title\": \"El pan integral adelgaza\", \"body\": \"Sacia más, pero las "
        "calorías cuentan\", \"footer\": \"...\"} (title = mito, body = realidad, both from narration).\n"
        "graphic_do_dont: {\"title\": \"EN EL DESAYUNO\", \"bad\": \"Pan blanco solo\", "
        "\"good\": \"Integral con proteína\", \"footer\": \"...\"} (bad = worse choice, good = better choice).\n"
        "graphic_recipe_snapshot: {\"title\": \"MERIENDA REAL\", \"items\": [\"Yogur natural\", "
        "\"Avena\", \"Fruta\"], \"footer\": \"...\"} (2-3 real foods).\n"
        "graphic_warning: {\"title\": \"EVITA ESTO\", \"items\": [\"Comprar con hambre\", "
        "\"Fiarte del color\"], \"footer\": \"...\"} (2-4 items to avoid; calm tone, no fear words).\n"
        ...
        "- surface_style: none | soft_card | editorial | plate_focus | hero_stat | binary_split "
        "| numbered_photo_bands | annotated_object | photo_tiles\n"
        ...
        "PREFERRED SURFACE STYLES (content-first system):\n"
        "- graphic_stat -> hero_stat (one enormous number as the hero).\n"
        "- graphic_do_dont / graphic_comparison / graphic_myth -> binary_split (two contrasting halves).\n"
        "- graphic_checklist / graphic_warning / graphic_step_list -> numbered_photo_bands when the "
        "subject has concrete photographable items (foods, objects): 2-4 horizontal edge-to-edge photo "
        "bands, each with one large circular numbered badge and a label of at most four words. "
        "numbered_photo_bands is ONLY valid on these three list layouts.\n"
        "- graphic_label_callout / graphic_plate_ratio -> annotated_object (callouts on the real object).\n"
        "- graphic_recipe_snapshot -> photo_tiles (labelled real-food tiles).\n"
        "Use visual variants deliberately so consecutive graphics do not look identical; two graphic "
        "scenes in the same Short should use different semantic or surface families.\n"
        "Keep graphic title 2-5 words, es-ES. Prefer max 4 words when possible. "
        "Checklist items: 2–4, hard limit. Label callouts: 2–3 preferred, 4 max. "
        ...
        "Use graphic_step_list with per-step \"time\" when the scene splits a routine into time blocks:\n"
        "10 min, 20 min, 30 min, cerrar el día, preparar dormitorio, respirar.\n"
        "Use graphic_stat when the narration centers on ONE memorable number, including an evidence\n"
        "number/fact tied to age/health that needs documentary credibility.\n"
        "Use graphic_myth when the narration corrects a misconception (mito -> realidad).\n"
        "Use graphic_do_dont when one option is clearly WORSE and one clearly BETTER;\n"
        "keep graphic_comparison for two NEUTRAL options.\n"
        "Use graphic_recipe_snapshot for a concrete practical meal example with 2-3 real foods.\n"
        "Use graphic_warning when the narration lists concrete mistakes/risks to avoid.\n"
        "Prefer variety: across the Short, do NOT default every card to a checklist — pick the\n"
        "layout that matches the content shape (a number -> graphic_stat, a contrast ->\n"
        "graphic_do_dont or graphic_comparison, a myth -> graphic_myth). Never reuse the same\n"
        "title on two different graphic scenes.\n\n"
...
def gemini_scenes_qa_prompt(channel_config: dict, short_script: dict, short_scenes: dict) -> str:
    ...
    return (
        ...
        "All strings must be valid JSON strings.\n\n"
        "QA RULES:\n"
        "SOURCE FIDELITY POLICY (judge by CONTENT COVERAGE, not exact wording):\n"
        "- Do NOT require the scene narration to match the script word-for-word. Faithful "
        "paraphrase or light rewording that keeps ALL required content is CORRECT — do not "
        "raise a source_fidelity issue for wording differences alone.\n"
        "- A script sentence SPLIT across two consecutive scenes is fine, as long as the two "
        "scenes together preserve every clause and no content is lost.\n"
        "- Raise a MAJOR source_fidelity issue ONLY when the scenes DROP or CHANGE THE MEANING "
        "of required content: a claim, a qualifier that changes the point (e.g. 'realmente', "
        "'sin cambiar nada'), the payoff sentence, a numbered step, a hook sentence, or the "
        "CTA's specific topic phrase. Dropping content = hard-fail; rewording that keeps it = pass.\n"
        "- Check COVERAGE: every meaningful script beat (hook sentences, each step, the payoff, "
        "the CTA topic) must appear across the scenes. If a beat is missing, fail source_fidelity "
        "and instruct SPLITTING/adding a scene to restore it — never instruct compressing it away.\n\n"
        ...
        "  - graphic_stat (ONE memorable number/quantity)\n"
        "  - graphic_myth (mito -> realidad correction)\n"
        "  - graphic_do_dont (worse choice vs clearly better choice)\n"
        "  - graphic_recipe_snapshot (2-3 real foods as a practical example)\n"
        "  - graphic_quote_portrait (one featured emotional sentence)\n"
        "  - graphic_evidence_nugget (credible age/health number/fact)\n"
        "  - graphic_warning (2-4 mistakes/risks to avoid, calm tone)\n"
        "- Do not allow long-form layouts. Do not allow any layout that is neither a\n"
        "  short_* layout nor one of the allowed graphic_* layouts above.\n"
        "- For graphic_plate_ratio: layout_payload.segments must have 2-4 entries whose\n"
        "  values sum to 100.\n"
        "- For graphic_checklist: layout_payload.items must have 2-4 short entries (a fifth item is invalid; the scene must be split or simplified).\n"
        ...
        "- For graphic_stat and graphic_evidence_nugget: layout_payload.title (the number/fact) and body (short label/context) must be non-empty.\n"
        "- For graphic_myth: layout_payload.title (mito) and body (realidad) must be non-empty and supported by narration.\n"
        "- For graphic_do_dont: layout_payload.bad and good must be non-empty; use graphic_comparison instead when the options are neutral.\n"
        "- For graphic_recipe_snapshot: layout_payload.items must have 2-3 real foods.\n"
        "- For graphic_quote_portrait: layout_payload.title is the featured sentence (max 60 chars).\n"
        "- For graphic_warning: layout_payload.items must have 2-4 entries and must avoid veneno, prohibido, nunca, milagro, cura, and \"doctores no quieren\".\n"
        "- Do not reject graphic scenes for omitting variant, visual_tone, background_mode, or surface_style; these fields are optional.\n"
        "- If graphic visual fields are present, reject invalid values. Allowed variant values: brand_default, warm_olive, soft_clay, cream_focus, evening_calm. Allowed visual_tone values: calm, focus, warning_soft, positive, evening. Allowed background_mode values: clean, radial, paper, video_blur. Allowed surface_style values: none, soft_card, editorial, plate_focus, hero_stat, binary_split, numbered_photo_bands, annotated_object, photo_tiles. numbered_photo_bands is only valid on graphic_checklist, graphic_warning, and graphic_step_list.\n"
        ...
        "- Fail graphic_stat or graphic_evidence_nugget scenes longer than 4.0 sec; fail graphic_myth, graphic_quote_portrait, or graphic_warning scenes longer than 4.5 sec.\n"
        ...
        "- MISSING GRAPHIC is a required change for checklist/explainer Shorts when graphic_count is 0, or when a structured short_* scene should clearly be graphic_* and graphic_count is below 2.\n"
        "- Do not add a third graphic if the Short already has 2 graphics; suggest improving the stock visual_prompt instead.\n"
        "- MISSING GRAPHIC: if a scene's narration contains a\n"
        "  compact visualizable structure but uses a stock short_* layout, add a\n"
        "  required change suggesting the matching graphic layout. Trigger ONLY on real\n"
        ...
        "  por 100 g), portion/component terms (porción, palma, plato, hidratos), two-choice comparisons (mejor/cuidado, opción A vs opción B),\n"
        "  or routine time blocks. Do NOT trigger on the bare words lista/regla/paso/minutos\n"
        "  without that structure. Suggested fixes can include convert_to_graphic_checklist,\n"
        "  convert_to_graphic_plate_ratio, convert_to_graphic_label_callout,\n"
        "  convert_to_graphic_comparison, convert_to_graphic_routine_split,\n"
        "  convert_to_graphic_stat, convert_to_graphic_myth, convert_to_graphic_do_dont,\n"
        "  convert_to_graphic_recipe_snapshot, convert_to_graphic_evidence_nugget,\n"
        "  or convert_to_graphic_warning.\n"
        ...
        "- Do not require lifestyle footage if a ChatGPT-generated graphic/card is clearer for the viewer.\n"
