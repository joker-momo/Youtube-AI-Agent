...
def short_scene_prompt_v6(
    ...
    *,
    ...
    return (
        ...
        "If narration is too dense, keep the step's CONVERSATIONAL action sentence (verb + object + the ONE key qualifier that makes it clear) and move only the EXTRA detail to caption, on_screen_text, visual action, or layout_payload.\n"
        "⚠️ NEVER reduce a step to a bare 2-4 word label. Example of a BAD compression that fails QA: script 'Primero, mide cuánto aceite usas realmente, sin cambiar nada' → 'Mide lo que usas.' (loses 'realmente' and 'sin cambiar nada', the whole point). A GOOD compression keeps it: 'Mide cuánto aceite usas, sin cambiar nada.'\n"
        "You may shorten scene.narration to fit timing if:\n"
        "- the core meaning AND its key qualifier stay faithful,\n"
        "- no new claim is added,\n"
        "- only supporting detail moves to caption, on_screen_text, or layout_payload.\n"
        "Aim for the UPPER end of each layout's word cap (a natural 7-10 word sentence for tips), not the floor — telegraphic 3-4 word fragments read as low quality and fail the product-quality gate.\n"
