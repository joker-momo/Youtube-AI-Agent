...
def test_post_asset_qa_rejection_and_fallback():
    ...
    svc.core._ensure_asset = mock_ensure_asset

    # For _search_and_download to run candidates, it needs some mocked methods.
    # _is_contradictory stays on the service; the search core calls it back through
    # its owning-service reference.
    ...
    svc.core._rank_candidates = MagicMock(return_value=[
    ...
    asset = svc.core._search_and_download(providers=["pexels"], query="test", filters={}, ttl_hours=24, scene=scene, channel_id="channel", job_id="job", is_fallback=False, require_strict=False)
    ...
    assert len(svc.core.last_errors) == 1
    assert svc.core.last_errors[0]["error_type"] == "VisionQAFailure"
    assert "No person" in svc.core.last_errors[0]["message"]
