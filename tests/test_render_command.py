...
def test_build_remotion_commands_include_props_and_outputs(tmp_path):
    ...
    commands = build_remotion_commands(render_props, video_path)
...
def test_video_render_defaults_to_all_cpu_cores(tmp_path):
    render_props = tmp_path / "render_props.json"
    render_props.write_text("{}", encoding="utf-8")
    commands = build_remotion_commands(render_props, tmp_path / "video.mp4")
...
def test_video_render_passes_gl_backend(tmp_path):
    render_props = tmp_path / "render_props.json"
...
# NOTE: no thumbnail-composition test remains here. The Remotion-still thumbnail
# render path (ThumbnailStandard composition, RemotionCommands.thumbnail,
# remotion/src/Thumbnail.tsx) was removed 2026-06-30 — the ChatGPT-generated
# thumbnail (auto_thumbnail_image_stage) is now the only thumbnail source. See
# render_with_remotion's ChatGPT-thumbnail-required check in render.py.
